    ...
    class BackgroundController {
      ...
      static async _onRunningGameChanged(isGameRunning) {
        if (isGameRunning) {
          ...
          // Open in-game window
          await WindowsService.restore(WindowNames.TIMER);
          ...
          WindowsService.close(WindowNames.TIMER);
        }
      }
      ...
      static async _restoreLaunchWindow() {
        ...
        if (!isGameRunning) {
          ...
          await WindowsService.restore(WindowNames.TIMER);
          WindowsService.minimize(WindowNames.TIMER);
        }
      }
      ...
      static _registerHotkeys() {
        hotkeysService.setToggleHotkey(async () => {
          const state = await WindowsService.getWindowState(WindowNames.TIMER);
          if (state === "minimized" || state === "closed") {
            WindowsService.restore(WindowNames.TIMER);
